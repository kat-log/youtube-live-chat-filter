...
> **これは 2026-09-05 時点の調査・実装メモで、再設計（フェーズ0〜9、2026-09-07〜08）より前の文書です。**
> セレクタは現在 `dom-chat.js` の `SELECTORS` レジストリに集約されているので、
> この文書の中の生のセレクタ文字列を実装の根拠にしないこと。
> 現状は [`architecture.md`](./architecture.md) の「DOMモードの読み取り」が正です。
