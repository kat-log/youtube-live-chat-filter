...
**v2.0.0 として片付けた（2026-09-08）。** 手順と確認項目は
[`release-checklist.md`](release-checklist.md) に移した（次のリリースでも使う形にしてある）。
やったこと: バージョンを 2.0.0 に上げ（`manifest.json` と `README.md` を対で。
食い違いは `test/manifest.test.js` が落とすようにした）、`CHANGELOG.md` を新設し、
掲載文と更新の告知文（**「権限が減ったこと」を先頭に置いた**）と
ストア審査用の権限の説明を `store-listing.md` に用意し、
`youtube-live-chat-filter-v2.0.0.zip` を作った。

**残っているのは人にしかできない2つだけ**（`release-checklist.md` の C）。
- **実配信での1時間以上の運転**（DOMモードとAPIモードの両方。権限を絞った直後なので必須）
- **スクリーンショットの差し替え**と、ダッシュボードへの貼り付け・提出

偽 YouTube + 実 Chromium の確認（同 B）は14項目すべて自動で通した。
そこで分かった但し書きが1つ: `chrome.permissions.getAll()` の `origins` は
**`https://www.googleapis.com/*` とホスト単位に丸められる**。
manifest にパスまで書いても付与はホストまで、という「権限は宣言でもある」の節の裏取りになる。
