...
- **popup が IndexedDB を開くようになった。** いまは同じ `DB_VERSION` を見ているので
  ...
  `onblocked` で reject するようになっている。ポート化（フェーズ6）で popup の
  生存期間が Service Worker から見えるようになるので、そのとき手当てを考えるとよい。
- **アバターの保持枠はフェーズ6 の宿題**（上の7）。`monitoringState` を
...
- **ポート化（フェーズ6 のやること9）は、この描画方式と相性がよい。**
...
- **`waitForServiceWorker` の8回 ping はまだ残っている**（`popup:` の初期化）。
  フェーズ6 で消える予定のもの。テストハーネスの `setTimeout` が「積むだけ」なのは
