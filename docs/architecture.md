...
`alt` に入る値は2種類あり、扱いを分ける必要がある。

| 種類 | `alt` | 画像 | 扱い |
| --- | --- | --- | --- |
| Unicode の絵文字 | 絵文字そのもの（🎉） | あってもなくてもよい | **文字のまま**。画像に戻す必要が無い |
| メンバー限定絵文字・`:yt:` などの標準絵文字 | 短縮名（`:_hearts:`） | 必須 | 短縮名 → 画像URL の対応表を持たせ、**表示側で画像に戻す** |

`extractMessageContent()` が `#message` の子節点を1回だけ走査して、
本文の文字列と対応表（`emojis`）を同時に作る。判定は「`alt` が短縮名の形か」
（`shared/comment.js` の `isEmojiShortcut()`）だけで、YouTube 側の属性は見ていない。

- **本文の文字列は書き換えない。** 短縮名のまま残す。ID の元になるキー
  （`commentKeyOf()`）と検索対象（`searchText`）が本文から作られているので、
  画像URLを本文に混ぜると**同じコメントのIDが配信ごとにぶれて履歴が二重になる**
  （スーパーステッカーの `stickerUrl` をキーに混ぜないのと同じ理由）
- 対応表は**使っている行にだけ**生やす。1件あたり数十バイトでも、履歴は
  1動画で数万件になりうる（`kind` を通常のコメントに載せないのと同じ理由）
- 同じ絵文字を何度使っても対応表は1つ。種類数の上限は16
  （`normalizeEmojis()`。保存に載る値なので形と件数はここで揃える）
- 要求するサイズは表示（24px）の2倍。`:yt:` などは youtube.com の静的ファイルで
  サイズ指定を受け付けないため、`/s/` 以下のパスだけをそのまま通す
- **APIモードには対応表が付かない。** `displayMessage` には短縮名しか入らず、
  画像URLは返ってこないので、APIモードでは従来どおり `:_hearts:` と表示される

表示側（`popup.js` の `messageNode()`）は本文を短縮名で切り分け、対応表にあって
**ホストの確認も通った**ものだけを `img` に差し替える。差し替えないものは文字のまま
残るので、対応表を持たない履歴（更新前に保存したもの・APIモード）でも本文は欠けない。
画像が読めなかったときも短縮名の文字に戻す（ステッカーと違い、本文の一部だから）。
