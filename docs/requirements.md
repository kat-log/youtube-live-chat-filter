...
> **これは 2025 年の初期MVPの記録であり、現在の実装とは異なる歴史文書です（#45）。**
> **この文書を根拠に実装判断をしてはいけません。** 現状は次を参照してください。
>
> - 現在の設計と約束事: [`../CLAUDE.md`](../CLAUDE.md)
> - 何が壊れていたか / どう直したか: [`audit-2026-09.md`](./audit-2026-09.md) と
>   [`redesign-plan.md`](./redesign-plan.md)
> - 利用者向けの機能一覧: [`../README.md`](../README.md)
>
> 現在の実装と食い違っている主なところ（2026-09 時点）:
>
> | この文書の記述 | 現状 |
> | --- | --- |
> | 2.1「特別アカウントのコメントのみを抽出」 | 取り込みは**全件**で、フィルターは表示側だけに掛かる（再設計の決定1）。`normal`（一般）のトグルもある |
> | APIモード専用（DOMモードへの言及なし） | **DOMモードが既定**で、APIキーは不要。APIモードは任意機能 |
> | 種別（スパチャ・メンバーシップ）への言及なし | v1.11.0 で追加。フィルターの軸が「役割」と「種別」の2本ある |
> | 3「MVP ではユーザー設定の保存は API キー以外は実装しません」 | `theme` / `timeHour12` / `timeShowSeconds` / `autoStart` / `chatMode` / `commentFilters` を保存している |
> | 5.2 データフロー「SW → content script → popup」 | SW から popup へポートで直接送る（フェーズ6b）。content script は経由しない |
> | 履歴は `chrome.storage.local` の配列 | IndexedDB（`src/shared/store.js`）。保持枠が2つある（決定2・決定3） |
