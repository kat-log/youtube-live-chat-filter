...
> **この文書を根拠に実装判断をしてはいけません。** アバターの保存はフェーズ6a で
> IndexedDB の `avatars` ストアと保持枠（`AVATAR_LIMITS`）に変わっています。
> 現状は [`architecture.md`](./architecture.md) の「コメント履歴の保存（IndexedDB）」が正です。
...
> **この節（§1）は実装前（2026-08-26 時点）の「現状」の記述です（#46）。**
> 行番号もその時点のもので、いまのコードとは対応しません。アバターは既に
> 両モードで実装済みで、DOMモードは `dom-chat.js` の `extractAvatarUrl()` が
> `#author-photo img` の `.src` から拾い、保存は `shared/store.js` の
> `avatars` ストア（保持枠つき）が持ちます。
> **§2 以降（CSP・容量分析・発言者ごとのマップという設計と結論）は現在も有効です。**
> §3.1 が自認している欠点（同名ユーザーのアバターが混ざる）も現在も残っています。
...
3. **再描画コスト（実測済み・要対処）**
   ...
   Chromium で実測したところ、ボトルネックは `innerHTML` ではなく
   **全行のスタイル計算とレイアウト**だった（2000件で全体の約95%）。

   | 方式 | 2000件の1回あたり | 現行比 |
   |---|---:|---:|
   | A. 現行（全件再構築） | 1069 ms | — |
   | B. 現行 + `content-visibility: auto` | 67 ms | **16倍速** |
   | C. 差分描画（新着5件のみ追加） | 8.2 ms | 130倍速 |
   | D. C + `content-visibility` | 1.6 ms | 660倍速 |

   ※ 計測機のCPUは一般的なPCの約5〜8倍遅い（単純ループのベンチで校正）。
   実機では概ねこの1/5〜1/8、つまり現行Aは**150〜200ms程度**と読む。
   内訳（2000件・現行CSS）: 文字列生成 11ms / `innerHTML`適用 25ms /
   リスナー付与 2ms / **レイアウト確定 1122ms**。
   `.comment-item:has(...)` の寄与は1割程度で主犯ではない。

   → **結論: B の1行だけはアバター実装と同時に入れる。**

   ```css
   .comment-item { content-visibility: auto; contain-intrinsic-size: auto 60px; }
   ```

   アバターは行を重くする方向の変更なので、この1行を先に入れておくと
