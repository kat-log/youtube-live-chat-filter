...
> **これは 2025-09-11 時点の調査結果で、現在の既定構成とは前提が違います。**
> いまの既定は **DOMモード（APIキー不要）** で、ここに書かれた quota の制約は
> APIモードを選んだときにだけ効きます。**この文書を根拠に既定の挙動を判断しないこと。**
> 現状は [`architecture.md`](./architecture.md) と [`../CLAUDE.md`](../CLAUDE.md) が正です。
