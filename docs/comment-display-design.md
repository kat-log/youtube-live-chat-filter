...
> **これは 2026-08-26 時点の調査・設計の記録で、再設計（フェーズ0〜9、2026-09-07〜08）より前の文書です。**
> **この文書を根拠に実装判断をしてはいけません。** とくに描画の方式は
> フェーズ5で「1コメントにつき1行を1回だけ作り、切り替えは `hidden` だけ」に変わっています。
> 現状は [`architecture.md`](./architecture.md) の「一覧の描画」と
...
> **この節（§1）は実装前（2026-08-26 時点）の記述です（#46）。**
> 行番号もその時点のもので、いまのコードとは対応しません。
> ここに挙がっている項目（フォントスタック・太字・時刻トグル）は実装済みで、
> 描画方式もフェーズ5 で「1コメントにつき1回だけ行を作る」形に変わっています。
> **§2 以降の設計と結論（font-weight とフォントスタック、時刻を文字列で
> 持たないこと、役割色）は現在も有効で、維持する対象です。**
...
## 8. 決定事項（2026-08-26 合意・実装済み）

| # | 論点 | 決定 |
|---|---|---|
| 1 | 本文 13px → 14px | **上げる**（font-weight 500 / line-height 1.6 とセット） |
| 2 | 発言者名の役割色 | **入れる。** あわせて一覧の役割バッジをアイコン化（👑 配信者 / 🔧 モデレーター / ⭐ メンバー、一般はバッジ無し）。ドロワーのフィルター側は文字バッジのまま |
| 3 | 12時間表記の区切り | `10:34 PM`（**スペースあり**）。詰めたい場合は `formatTimestamp()` の1文字を消すだけ |
| 4 | Latin/数字用フォントの同梱 | **見送り。** 実行時コストはほぼゼロ（ローカル読み込み20〜40KB）だが、時刻は既に等幅・tabular-nums で揃っており改善幅が小さい。加えてサブセット生成という**ビルド工程が要る**のが最大の難点（この拡張は「ビルド不要」が売り）。効果に対して構成が重くなるので採らない |

### 実装時に設計から変えた点

- 役割色は `:has()` で直接 `color` を当てるのではなく、行に
  **カスタムプロパティ `--role-color` を立てて継承させる**形にした。
  `.comment-author.selected`（青背景＋白文字）との詳細度争いが起きず、
  左端3pxラインも同じ変数を使えて記述が減る
- ライトテーマの役割色は**専用の暗いカットを追加**した
  （`--sponsor: #4ade80` は白背景で 1.8:1 しか出ない → `#15803d` で 4.9:1）
- 一般コメントは `.role-normal` の要素自体を出さなくなったため、
  左ラインは `var(--role-color, var(--normal))` のフォールバックで塗る
- `.comment-author` のライトテーマ常時グレー背景を廃止し、hover時のみに変更
  （役割色とグレーチップが重なって濁るため。ダークテーマと挙動も揃う）
- 時刻設定の反映経路は `chrome.storage.onChanged` に一本化。
  ドロワーのトグルは `storage.set` するだけで、再描画には関与しない
- `loadTimeSettings()` は `completeBasicInitialization()` の
  `Promise.all` に**混ぜず、その前で await** する。
  並列にすると履歴復元の描画と競合し、一瞬だけ旧形式で描かれてそのまま残る

### 実装後の検証で見つけて直したもの

ライトテーマで**トグルのON/OFFが判別できなかった**（既存のバグ）。

`.toggle-slider` の地色は `rgba(255,255,255,0.1)`、ノブは
`rgba(255,255,255,0.5)` で、どちらも白系。ダークテーマ前提の値なので
ライトテーマでは背景に溶ける。さらに `input:checked` でトラック色が変わるのは
`.owner-slider` など**役割クラスを持つトグルだけ**で、ダークモードのトグルには
ON色が定義されていなかった。つまりライトテーマのダークモードトグルは
以前からON/OFFを見分けられない状態だった。

今回そこへ時刻の2トグルを足すと、この欠陥が3箇所に増えて実害が出るため、
あわせて修正した:

- `input:checked + .toggle-slider` に `var(--accent)` のON色を追加
  （役割スライダーと同じ詳細度なので、**必ずその上の行に置く**。
  下に置くと役割色を上書きしてしまう）
- ライトテーマ用に `.toggle-slider` の地色を `rgba(0,0,0,0.18)`、
  ノブを `#fff` に差し替え

### 残った確認事項

コンテナ上のヘッドレスChromiumでレイアウト・配色・折り返しは確認済み
（ダーク／ライト、24h／12h、秒あり／なしの各組み合わせ）。ただし
**「太さ」の見え方だけは実機確認が必要**：Linuxコンテナにはヒラギノも
游ゴシックも無く、合成ボールドで描かれるため §2 の効果を再現できない。
macOS と Windows の実ブラウザで確認すること（§6.4）。
