...
**バージョン:** v2.1.3（正は [`src/manifest.json`](src/manifest.json) の `version`。
...
- 発言者名クリックでその人だけに絞り込み
  （YouTubeのチャット側でも、名前やアイコンの **Alt+クリック** で同じ絞り込みができます）
...
気になった人だけを見たいときは、**YouTubeのチャットでその人の名前かアイコンを
Alt+クリック**すると、その人で絞り込んだ状態でポップアップが開きます
（素のクリックは YouTube の「ブロック」「報告」のメニューのままです）。
