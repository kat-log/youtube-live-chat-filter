...
(function (global) {
  ...
  //
  // 全件取り込み（フェーズ4）で初めて実際の流量が乗るので、そこで見積もり直した。
  // 日本語のコメント1件の保存レコードは JSON でおよそ 375 バイト
  // （pk・videoId・seq・bucket と searchText を含む実測値）。
  //   primary 20,000 件 ≒ 7 MiB / bulk 50,000 件 ≒ 18 MiB（動画1本あたり 25 MiB）
  //   保持5本ぶんを両枠とも上限まで使った最悪ケースで 125 MiB
  // 賑わった配信で毎分200件としても bulk が埋まるまで4時間ある。
  // unlimitedStorage 権限があるので上限そのものは通るが、これ以上大きくすると
  // 「使っていない過去の配信」で数百MiB を占め始めるため、ここで止める
