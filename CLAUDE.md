...
全件）を対象にする。比較の前に `normalizeForSearch()` で NFKC 正規化・小文字化・
前後の空白除去を通すので、IME由来の全角/半角の揺れ（`！` と `!`、`ｶﾅ` と `カナ`）や
変換後に紛れ込む末尾スペースでは取りこぼさない。正規化済みの文字列はコメントごとに
1度だけ作って持たせる（1文字打つたびに全件を正規化し直すと数千件で遅くなる）。
