...
- **チャットの名前・アイコンの「素のクリック」は YouTube のもの。**
  クリックすると「ブロック」「報告」のメニューが開く。奪うと本体の機能が壊れるので、
  こちらが受け取るのは修飾キー付き（`USER_FILTER_MODIFIER`）だけにして、
  そのときだけ捕捉フェーズで止める。
- **行の中の画像（ステッカー・アバター）は、行がDOMに入った直後にはまだ無い。**
  `yt-img-shadow` があとから `img` を作るので、その場で読むと空になる。
  **アバターを「時間で待って」取ろうとしない。** いつ生えるかは YouTube の都合
  （ビューポートに入ってから読み込む）で上限が無く、0.5秒待っても半分は
  間に合わなかった。取り込みは待たせず、別便で追いかける（`pendingAvatars`）。
  待つのは**ステッカーだけ**——名前（`alt`）が本文そのもので、IDも本文から作るため。
  その待ちは1本の列（`pendingRows`）にまとめる。行ごとに待たせると、待っている行を
  あとの行が追い越して popup の並びが前後する。
...
- **ツールバーの popup は、ページをクリックした時点でもう閉じている**
  （フォーカスが外れると必ず閉じる Chrome の仕様）。だから YouTube 側からの
  絞り込みは「開いている popup を切り替える」ではなく、**置いてから開き直す**の
  2手になる（`pendingUserFilter`。開くのは `chrome.action.openPopup()` で、
  Chrome 127 以降にしか無い）。置くのは**まだ届けていないクリック1件**で、
  絞り込みの状態そのものではない（正は popup の `selectedUser`）。
...
- **`bulk` 枠（メンバー・一般）を読むかどうかを `primary` の件数で決めない。**
  配信者・モデレーターの発言もスパチャも無い配信では `primary` が0件のままなので、
  「`primary` があれば数える」にすると保存済みのメンバー・一般が丸ごと復元されず、
  popup を開いている間に届いたぶんだけが並ぶ（閉じて開き直すとまた消える）。
  `restoreCommentHistory` は件数に関わらず `loadBulkCount()` まで通す。
